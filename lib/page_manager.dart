...
class PageManager {
  ...
  final currentBackgroundNotifier = ValueNotifier<String>('');
  final currentArtistNotifier = ValueNotifier<String>('');
  ...
  final _channel = getIt<IcePlaylist>().channel;
  ...
  void add() async {
    final songRepository = getIt<IcePlaylist>();
  ...
  void _listenToChangesInChannel() {
    _channel.listen((update) {
      ...
      /*      if (update.icestats.source.title !=
        ...
        _audioHandler.addQueueItem(MediaItem(
          id: update.icestats.hashCode.toString(),
          artist: update.icestats.source.artist,
          title: update.icestats.source.title,
          artUri: Uri.parse(update.icestats.source.images[0]),
          extras: {
            'url': update.icestats.source.listenURL,
          },
        ));
  ...
  void _listenToChangesInSong() {
    _audioHandler.mediaItem.listen((mediaItem) {
      ...
      //currentBackgroundNotifier.value = playlist.nowPlaying.song[0].images?[0] ?? '';
      _updateSkipButtons();
    });
  }
